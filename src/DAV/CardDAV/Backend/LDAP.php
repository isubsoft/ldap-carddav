...
class LDAP extends \Sabre\CardDAV\Backend\AbstractBackend implements \Sabre\CardDAV\Backend\SyncSupport {
     ...
     * @var string
     */
    private static $backendSyncTableName = 'cards_backend_sync';
    ...
    /**
     * Address books for which housekeeping has been done
     *
     * @var array
     */
    private $housekeepingDone = [];
    ...
    /**
     * Array of PDO prepared statements
     *
     * @var array
     */
		private $getAddressBooksPdoPrepStmt = [];
     ...
     * @var array
     */
		private $housekeepingPdoPrepStmt = [];
     ...
     * @var array
     */
		private $generateVcardPdoPrepStmt = [];
    ...
    /**
     * Populate the backend properties of an address book
    **/    
    private function setAddressbookBackendProperties($addressBookId)
    {
    		if(isset($this->addressbook[$addressBookId]['LdapConnection']) && isset($this->addressbook[$addressBookId]['addressbookDn']))
    			return;
    			
				$addressBookConfig = $this->config['card']['addressbook']['ldap'][$addressBookId];
        ...
        else
        {
          ...
          if(isset($addressBookConfig['search_base_dn']) && $addressBookConfig['search_base_dn'] != '' && isset($addressBookConfig['search_filter']) && $addressBookConfig['search_filter'] != '')
          {
            ...
            else
            {
          	...
          	$addressBookDn = Utility::replacePlaceholders($addressBookConfig['base_dn'], ['%u' => ldap_escape($this->principalId, "", LDAP_ESCAPE_DN)]);
      	}
		...
		function getAddressBooksForUser($principalUri)
    {
      ...
      foreach ($this->config['card']['addressbook']['ldap'] as $addressBookId => $addressBookConfig) {
      	// Skip address book configuration if not enabled for type of principal
				if(((!isset($addressBookConfig['shared']) || !is_bool($addressBookConfig['shared']) || $addressBookConfig['shared'] === false || !isset($addressBookConfig['user_specific']) || $addressBookConfig['user_specific'] !== true) && $isGroupPrincipal) || (isset($addressBookConfig['shared']) && $addressBookConfig['shared'] === true && !$isGroupPrincipal))
					continue;
      	...
      	if(!isset($this->addressbook[$addressBookId])) {
		      ...
		      $this->addressbook[$addressBookId]['syncToken'] = time();
		      $this->addressbook[$addressBookId]['syncDbUserId'] =  ($addressBookConfig['user_specific'])?$this->principalBackendId:$systemUser;
	      ...
	      if(!in_array($addressBookId, $this->housekeepingDone)) {
 					$this->housekeeping($addressBookId);
 					$this->housekeepingDone[] = $addressBookId;
 				}
    ...
    function getCard($addressBookId, $cardUri)
    {
		    $result = [];
				...
				if(!isset($this->getCardPdoPrepStmt['stmt01'])) {
					// Preparing PDO statements which are used inside a loop
					try {
						$query = 'SELECT card_uid, backend_id FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND card_uri = ? AND delete_sync_token IS NULL';
						$this->getCardPdoPrepStmt['stmt01'] = $this->pdo->prepare($query);
					}
					catch (\Throwable $th) {
						trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
						throw new SabreDAVException\ServiceUnavailable();
				...
				$stmt01 = $this->getCardPdoPrepStmt['stmt01'];
        
				try 
				{
		      $stmt01->execute([$syncDbUserId, $addressBookId, $cardUri]);
					$row = $stmt01->fetch(\PDO::FETCH_ASSOC);
		    } catch (\Throwable $th) {
					trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
					throw new SabreDAVException\ServiceUnavailable();
    ...
    protected function createUpdateCard($addressBookId, $cardUri, $cardData, $operation = 'CREATE')
    {
        ...
        if(!$addressBookConfig['writable'])
				...
				foreach($vcard->validate() as $validationError)
					if($validationError['level'] >= 3)
						throw new SabreDAVException\BadRequest("Validation error for card property '" . ($validationError['node'])->name . "'. Make sure version property is mentioned in the card data and card data is formatted according to the version.");
					
				$vcard = $vcard->convert($this->defaultVcardVersion);
	      $cardUid = (!isset($vcard->UID) || $vcard->UID == null || $vcard->UID == '')?null:$vcard->UID;
        ...
        if($operation == 'CREATE')
        {
		      ...
		      try {
		          $query = 'SELECT 1 FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND card_uid = ? AND delete_sync_token IS NULL';
		          $stmt = $this->pdo->prepare($query);
		          $stmt->execute([$syncDbUserId, $addressBookId, $cardUid]);
        ...
        if($operation == 'UPDATE')
        {
		      ...
		      try {
		          $query = 'SELECT 1 FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND card_uri = ? AND card_uid <> ? AND delete_sync_token IS NULL';
		          $stmt = $this->pdo->prepare($query);
		          $stmt->execute([$syncDbUserId, $addressBookId, $cardUri, $cardUid]);
        ...
        if($isContactGroup)
        {
          ...
          $rdnAutorename = (!isset($addressBookConfig['group_LDAP_rdn_autorename']) || !is_bool($addressBookConfig['group_LDAP_rdn_autorename']))?false:$addressBookConfig['group_LDAP_rdn_autorename'];
          $rdnAutorenameReplace = (!isset($addressBookConfig['group_LDAP_rdn_autorename_replace']) || !is_bool($addressBookConfig['group_LDAP_rdn_autorename_replace']))?false:$addressBookConfig['group_LDAP_rdn_autorename_replace'];
        ...
        else
        {
          ...
          $rdnAutorename = (!isset($addressBookConfig['LDAP_rdn_autorename']) || !is_bool($addressBookConfig['LDAP_rdn_autorename']))?false:$addressBookConfig['LDAP_rdn_autorename'];
          $rdnAutorenameReplace = (!isset($addressBookConfig['LDAP_rdn_autorename_replace']) || !is_bool($addressBookConfig['LDAP_rdn_autorename_replace']))?false:$addressBookConfig['LDAP_rdn_autorename_replace'];
            ...
            foreach(((isset($addressBookConfig['group_member_map']) && is_array($addressBookConfig['group_member_map']))?$addressBookConfig['group_member_map']:[]) as $vCardKey => $ldapKey) 
            {
                ...
                if(isset($vcard->$vCardKey) && $multiAllowedStatus['status'] && !$compositeAttrStatus['status'] )
                {
                    $newLdapKey = strtolower($ldapKey['field_name']);
										...
										// Preparing PDO statements which are used inside a loop
										try {
		                  $query = 'SELECT backend_id FROM ' . self::$backendMapTableName . ' WHERE addressbook_id = ? and card_uid = ? and user_id = ? AND delete_sync_token IS NULL';
		                  $stmt01 = $this->pdo->prepare($query);
										}
										catch (\Throwable $th) {
											trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
											throw new SabreDAVException\ServiceUnavailable();
                    ...
                    {                 
                        ...
                        if($memberCardUID != '')
                        {
                            ...
                            try {
                              $stmt01->execute([$addressBookId, $memberCardUID, $syncDbUserId]);
                              $row = $stmt01->fetch(\PDO::FETCH_ASSOC);
                    ...
                    if(isset($data[$newLdapKey]))
                    {
                        foreach($data[$newLdapKey] as $key => $value)
                        {
                            ...
                            if($memberData['count'] > 0)
                            {
																...
																if(!isset($this->generateVcardPdoPrepStmt['stmt01'])) {
																	// Preparing PDO statements which are used inside a loop
																	try {
														        ...
														        $this->generateVcardPdoPrepStmt['stmt01'] = $this->pdo->prepare($query);
																	}
																	catch (\Throwable $th) {
																		trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
																		throw new SabreDAVException\ServiceUnavailable();
																...
																$stmt01 = $this->generateVcardPdoPrepStmt['stmt01'];

                                $memberCardUID = null;

                                try {
                                    $stmt01->execute([$addressBookId, $memberData[0]['entryuuid'][0], $syncDbUserId]);
                                    $row = $stmt01->fetch(\PDO::FETCH_ASSOC);
     ...
     */
		...
		{     
			...
			$backendSyncToken = null;
			$fullSyncToken = null;
			
			try {
					$query = 'SELECT sync_token FROM ' . self::$backendSyncTableName . ' WHERE addressbook_id = ? AND user_id = ?';
					$stmt = $this->pdo->prepare($query);
					...
					if($row !== false)
						$backendSyncToken = (int)$row['sync_token'];
			...
			if($backendSyncToken == null)
	    	throw new SabreDAVException\ServiceUnavailable();
	    	
			$minSyncInterval = ($fullRefreshInterval / 10 > 1800)?1800:$fullRefreshInterval / 10;
			
			if($addressBookSyncToken - $backendSyncToken > $minSyncInterval)
			{ // Perform backend search for changes
				$this->setAddressbookBackendProperties($addressBookId);
				...
				try {
					...
					$stmt02 = $this->pdo->prepare($query);
					
					$query = 'INSERT INTO ' . self::$backendMapTableName . ' (user_id, addressbook_id, card_uri, card_uid, backend_id, create_sync_token)  VALUES (?, ?, ?, ?, ?, ?)';
					$stmt03 = $this->pdo->prepare($query);
				}
				catch (\Throwable $th) {
					trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
					throw new SabreDAVException\ServiceUnavailable();
				...
				$getChangesFromBackendFile = __TMP_DIR__ . '/' . __APP_NAME__ . '_' . strtolower(md5(implode('/', [self::$fullRefreshTableName, $syncDbUserId, $addressBookId]))) . '_process.lock';
				
				if(file_exists($getChangesFromBackendFile)) {
					trigger_error("Get backend changes for address book '$addressBookId' of sync database user '$syncDbUserId' is in progress in another local session. Client will be notified to retry after sometime. If this message continue to appear after a few retries by the client, the following local file may have to be manually removed - '$getChangesFromBackendFile' and this may require the server process to be stopped.", E_USER_NOTICE);
				...
				$getChangesFromBackendFileHandle = fopen($getChangesFromBackendFile, 'x');
					
				if($getChangesFromBackendFileHandle === false)
					throw new SabreDAVException\ServiceUnavailable();
				...
				while($data = Utility::LdapIterativeFetch($ldapConn, $data['entryIns'], $data['fetchFirst']))
				{
					...
					try {
						$stmt01->execute([$syncDbUserId, $addressBookId, $backendId]);
						$row = $stmt01->fetch(\PDO::FETCH_ASSOC);
						...
						if($row !== false) {
							...
							$stmt03->execute([$syncDbUserId, $addressBookId, $cardUri, $cardUid, $backendId, time()]);
						}
					} catch (\Throwable $th) {
						trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
        ...
        try {
					$query = "UPDATE " . self::$backendSyncTableName . " SET sync_token = ? WHERE user_id = ? AND addressbook_id = ?"; 
					$sql = $this->pdo->prepare($query);
			...
			try {
				// Fetch new contacts
				$query = 'SELECT card_uri FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND delete_sync_token IS NULL AND create_sync_token >= ? AND create_sync_token < ?';
				$stmt = $this->pdo->prepare($query);
				...
				$query = 'SELECT card_uri FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND delete_sync_token IS NOT NULL AND delete_sync_token >= ? AND delete_sync_token < ?';
				$stmt = $this->pdo->prepare($query);
				$stmt->execute([$syncDbUserId, $addressBookId, $syncToken, $addressBookSyncToken]);
    ...
    protected function addChange($addressBookId, $objectUri, $operation = 'DELETE')
    {
        ...
        {
		      try {
		        $query = "UPDATE " . self::$backendMapTableName . " SET delete_sync_token = ? WHERE addressbook_id = ? AND card_uri = ? AND user_id = ? AND delete_sync_token IS NULL"; 
		        $sql = $this->pdo->prepare($query);
        ...
        {
		      try {
		        $query = "UPDATE " . self::$backendMapTableName . " SET modify_sync_token = ? WHERE addressbook_id = ? AND card_uri = ? AND user_id = ? AND delete_sync_token IS NULL"; 
		        $sql = $this->pdo->prepare($query);
		        $sql->execute([time(), $addressBookId, $objectUri, $syncDbUserId]);
		      } catch (\Throwable $th) {
						trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
    ...
    function fetchLdapContactDataByUri($addressBookId, $cardUri, $attributes = [], int $attributesOnly = 0)
    {
        ...
        try {
            $query = 'SELECT backend_id FROM ' . self::$backendMapTableName . ' WHERE addressbook_id = ? and card_uri = ? and user_id = ? AND delete_sync_token IS NULL';
            $stmt = $this->pdo->prepare($query);
    ...
    function getMappedContacts($addressBookId)
    {
        ...
        $syncDbUserId = $this->addressbook[$addressBookId]['syncDbUserId'];
				...
				{
					try {
						$query = 'SELECT card_uri, card_uid, backend_id FROM ' . self::$backendMapTableName . ' WHERE user_id = ? AND addressbook_id = ? AND delete_sync_token IS NULL';
						$stmt = $this->pdo->prepare($query);
        ...
        if($data === false) {
          ...
          $cardUri = null;
					...
					if($row === false) {
						...
						try {
							$stmt02->execute([$syncDbUserId, $addressBookId, $cardUri, $cardUid, $backendId, time()]);
						} catch (\Throwable $th) {
							trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
							fclose($fullRefreshLockFileHandle);
							unlink($fullRefreshLockFile);
    ...
    private function housekeeping($addressBookId)
    {
      ...
      $syncDbUserId = $this->addressbook[$addressBookId]['syncDbUserId'];
			...
			if(!isset($this->housekeepingPdoPrepStmt['stmt01'])) {
				...
				try {
					$query = 'SELECT sync_token FROM ' . self::$backendSyncTableName . ' WHERE addressbook_id = ? AND user_id = ?';
					$this->housekeepingPdoPrepStmt['stmt01'] = $this->pdo->prepare($query);
				}
				catch (\Throwable $th) {
					trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
					throw new SabreDAVException\ServiceUnavailable();
			...
			$stmt01 = $this->housekeepingPdoPrepStmt['stmt01'];
			
			try {
					$stmt01->execute([$addressBookId, $syncDbUserId]);
					$row = $stmt01->fetch(\PDO::FETCH_ASSOC);
					
					if($row === false)
					{
						$query = "INSERT INTO " . self::$backendSyncTableName . " (user_id, addressbook_id, sync_token) VALUES (?, ?, ?)"; 
						$stmt = $this->pdo->prepare($query);
			...
			if(!isset($this->housekeepingPdoPrepStmt['stmt02'])) {
				// Preparing PDO statements which are used either inside a loop or part of a function 
				// which is called repeatedly during processing of the request
				try {
					$query = 'SELECT sync_token FROM ' . self::$fullSyncTableName . ' WHERE addressbook_id = ? AND user_id = ?';
					$this->housekeepingPdoPrepStmt['stmt02'] = $this->pdo->prepare($query);
				}
				catch (\Throwable $th) {
					trigger_error("Caught exception. Error message: " . $th->getMessage(), E_USER_WARNING);
					throw new SabreDAVException\ServiceUnavailable();
			...
			$stmt02 = $this->housekeepingPdoPrepStmt['stmt02'];
			
			$fullSyncToken = null;
			
			try {
					$stmt02->execute([$addressBookId, $syncDbUserId]);
					$row = $stmt02->fetch(\PDO::FETCH_ASSOC);
