    ...
    function getPrincipalByPath($path)
    {
        ...
        if(strtolower($principalId) != strtolower($currentUserPrincipalId))
			  ...
			  if(!isset($this->config['principal']['ldap']['search_bind_dn']) || $this->config['principal']['ldap']['search_bind_dn'] == '') {
				...
				$principal = CacheMaster::decode($this->cache->get(self::getCacheKey($principalId), null));
        ...
        if(!empty($data) && $data['count'] === 1)
        {
            ...
            $principal['__backend_id'] = $data[0]['entryuuid'][0];
						...
						if(!$this->cache->set(self::getCacheKey($principalId), CacheMaster::encode($principal), (isset($this->config['cache']['principal']['ttl']) && is_int($this->config['cache']['principal']['ttl']) && $this->config['cache']['principal']['ttl'] > 0 && $this->config['cache']['principal']['ttl'] <= 2592000)?$this->config['cache']['principal']['ttl']:self::$cacheTtl))
